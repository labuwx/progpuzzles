...
def hash(input):
    ...
    dense_mem = [
        ft.reduce(lambda x, y: x ^ y, mem[i * 16 : i * 16 + 16]) for i in range(16)
    ]
...
disk = np.asarray(
    [
        [int(b) for b in format(hash(input + '-%d' % i), 'b').zfill(128)]
        for i in range(128)
    ]
)
num_ones = disk.sum()
...
regs = {
    (i, j): (i, j) for i, j in it.product(range(128), range(128)) if disk[i, j] == 1
}
change = True
while change:
    change = False
